...
public class NashCommunicationData extends CommunicationData {
    ...
    /**
     * Construct a NashCommunicationData object, which is used by the NashUStructure
     * class.
     * 
     * @param initialStateID The initial state's ID
     * @param eventID        The event's ID
     * @param targetStateID  The target state's ID
     * @param roles          The array of communication roles (sender, receiver, or
     *                       none)
     * @param cost           The cost of this communication
     * @param probability    The probability of choosing this communication (a value
     *                       between 0 and 1, inclusive)
     **/
    public NashCommunicationData(long initialStateID, int eventID, long targetStateID, CommunicationRole[] roles,
            double cost, double probability) {
    ...
    public NashCommunicationData clone() {
        return new NashCommunicationData(initialStateID, eventID, targetStateID, ArrayUtils.clone(roles), cost,
                probability);
    }
     ...
     * @return {@code true} if this Nash communication data is the same as the
     *         argument
     * 
     * @since 2.0
     */
    ...
    @Override
    public int hashCode() {
        return Objects.hash(super.initialStateID, super.eventID, super.targetStateID, Arrays.hashCode(roles), cost,
                probability);
    }
