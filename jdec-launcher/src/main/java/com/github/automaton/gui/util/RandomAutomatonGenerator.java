...
/**
 * Utility class used to generate random automata (with a number of specified
 * properties).
 *
 * @author Micah Stairs
 * @author Sung Ho Yoon
...
public class RandomAutomatonGenerator {
    ...
    /**
     * Generate a random automaton with the specified properties. The generated
     * automaton is guaranteed to
     * be both observable, controllable, and accessible, co-accessible.
     * NOTE: This process is terminated
     * 
     * @param prompt                 A reference to the prompt that started this
     *                               process
     ...
     *                               generation process
     * @return The randomly generated automaton (or {@code null} if the process was
     *         aborted)
     * 
     * @since 2.0
     **/
    public static Automaton generateRandom(RandomAutomatonPrompt prompt,
        ...
        // Repeat generation until it passes both the observability and controllability
        // tests
        while (prompt == null || !prompt.isDisposed) {
            ...
            /*
             * Create empty automaton with capacities that should prevent the need to
             * re-create the body file
             */
            ...
            while (!isAccessible) {
                ...
                // If possible, flip one random transition between the new states and the
                ...
                while (automaton.getBadTransitions().size() < nBadTransitions) {
                    ...
                    // NOTE: As long as maxTransitionsPerState * nStates is not less than
                    // nBadTransitions, then this
                    ...
                    outer: while (true) {
                        ...
                        for (Transition transition : transitions) {
                            ...
                            if (!automaton.isBadTransition(s, transition.getEvent().getID(),
                                    transition.getTargetStateID())) {
                                automaton.markTransitionAsBad(s, transition.getEvent().getID(),
                                        transition.getTargetStateID());
                                break outer;
                            }
                        ...
                        // If we have gotten this far, we should add a new transition to this state,
                        // marking it bad
                        // NOTE: We cannot do this if the maximum number of transitions has already been
                        // exceeded
            ...
            // If the observability or controllability properties are both satisfied, then
            // we are done
    ...
    /**
     * Add a random transition to the automaton, ensuring that no duplicates are
     * created.
     ...
     * @param targetStateID The ID of the state which the added transition will lead
     *                      to
     ...
     **/
    private static void updateProgressIndicator(final JLabel progressIndicator, final String text,
            final int nAttempts) {
    ...
    /**
     * Give a unique name to the event, based on its ID and the largest possible ID
     * (which is used to calculate how many letters are needed).
     * 
