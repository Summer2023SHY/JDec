...
public abstract class NashInformationPrompt extends JDialog {
    ...
    public NashInformationPrompt(JDec gui, JDec.AutomatonTab tab, String title, String message) {
        ...
        // Skip the screen used to choose communication costs and probabilities if there
    ...
    private void addComponents(String message) {
        ...
        for (CommunicationData data : potentialCommunications) {
            ...
            // Calculate default probability so that it is dispersed as equally as possible,
            // but so that
        ...
        // Add communications to the table which have pre-assigned probability and cost
        // values
        for (NashCommunicationData data : nashCommunications) {
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                // Don't allow the user to press this button more than once after they've
                // entered valid input
                if (pressedNext)
                    return;

                // Ensure that the user is not editing a cell (which means the typed value is
                // not yet saved to the table)
                ...
                try {
                    ...
                    gui.displayErrorMessage("Input Errors",
                            "Not all entered values could be interpreted as a number. Please fix all cells highlighted in red.");
                    return;
                }

                // Ensure that the column of probability adds up to exactly 1.0
                if (totalProbability != 1d) {
                    gui.displayErrorMessage("Probability Values",
                            "The sum of the probability values must equal exactly 1. The entered sum was '"
                                    + totalProbability + "'");
                    return;
                }
                ...
                // Re-add the communications to the U-Structure (now with updated Nash
                // communications)
                for (int i = 0; i < nCommunications; i++) {
                ...
                // Perform the action specified by the implementing class once the Nash
    ...
    /**
     * This action is performed once the user has validated all of the Nash
     * communications and moves on
     * to the next step.
     **/
